...
def win_recursive(board, alpha, beta, level=0):
    if (level == DEPTH):
        ...
        # if board[ROW_SIZE-1,parent_col] != -1 :
        #     changed_row = ROW_SIZE-1
        # else :
        #     changed_row = np.argmin(board[:,parent_col])-1
        # copy_board = np.copy(board)
        # copy_board[changed_row,parent_col] = 0 if board[changed_row,parent_col] == 1 else 1
        # if board[changed_row,parent_col] == 1 :
        #     return evaluate(board,1) - evaluate(copy_board,0)
        # else : 
        #     return evaluate(copy_board,1) - evaluate(board,0)
    ...
    for column in range(COLUMN_SIZE - 3):  # linetype \
        for row in range(3, ROW_SIZE):
            if childBoard[row][column] == level_modulo_2 and childBoard[row - 1][column + 1] == level_modulo_2 and \
                    childBoard[row - 2][column + 2] == level_modulo_2 and childBoard[row - 3][column + 3] == level_modulo_2:
                return False
...
def evaluate(board, player):
    ...
    for column in range(COLUMN_SIZE):  # linetype |
        ...
        total_point += pointByFeature(row, player,column)
...
def pointByFeature(row, player,column=-1):
    ...
    for idx, val in enumerate(row):
        ...
        if val != player and start_idx != -1:
            sum_point += getPoint(row, player, start_idx, idx - 1,column)
            start_idx = -1
    return sum_point
...
def getPoint(row, player, start_idx, end_idx,column):
    ...
    if chessman_num == 1:  # Feature4
        if column==-1:
            column=start_idx
        if front_is_empty or back_is_empty:
        ...
        if front_is_empty and (back_is_empty == False):  # Feature 3
            temp = start_idx - 2
